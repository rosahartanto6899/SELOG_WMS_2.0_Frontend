...
interface DashboardIncentiveDriverProps {
  paramPowerBI?: PowerBIParams | null;
}

const DashboardIncentiveDriverPage = ({
  paramPowerBI,
}: Readonly<DashboardIncentiveDriverProps>) => {
  const { t } = useTranslation(undefined, {
    keyPrefix: "dashboard.incentiveDriver",
  });
  console.log(paramPowerBI);
...
export async function getServerSideProps() {
  const reportId = decryptData(
    process.env.POWER_BI_REPORT_ID_INCENTIVE as string,
  );
  const pageId = decryptData(
    process.env.POWER_BI_PAGE_ID_INCENTIVE_DRIVER as string,
  );
