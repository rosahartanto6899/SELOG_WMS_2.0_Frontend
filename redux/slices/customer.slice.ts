...
import { Customer, GetCustomersResponse } from "@sera-types/customer.type";
...
export const customerState = createSlice({
  ...
  reducers: {
    ...
    getCustomersSuccess: (
      ...
      if (data) {
        state.data = data.map((c: Customer, index: number) => {
...
export const { actions: customerActions, reducer: customerReducers } =
  customerState;
