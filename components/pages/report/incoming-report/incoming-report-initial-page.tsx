...
// parity legacy IncomingReport.js: filter tanggal default hari ini, max 31 hari
import {
  DownloadOutlined,
  FileExcelOutlined,
  FileTextOutlined,
...
import {
  incomingReportActions,
  useAppDispatch,
  useAppSelector,
} from "@sera-redux";
import { BaseType } from "@sera-types/base.type";
import { incomingReportTypes } from "@sera-types/incoming-report.type";
...
import { Columns, SearchByOptions } from "./incoming-report-props-table";
...
const exportExcel = (rows: any[], t: any, subtitle: string) => {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "Incoming Report");
  XLSX.writeFile(wb, `incoming-report-${exportTimestamp()}.xlsx`);
};
...
const exportCsv = (rows: any[], t: any) => {
  ...
  a.href = url;
  a.download = `incoming-report-${exportTimestamp()}.csv`;
  a.click();
  URL.revokeObjectURL(url);
};

const IncomingReportInitialPage = () => {
  const dispatch = useAppDispatch();
  const { t } = useTranslation(undefined, {
    keyPrefix: "report.incomingReport",
  });

  const { data, options } = useAppSelector((state) => state.incomingReport);
  const loading = useAppSelector(
    (state) => state.loading[incomingReportTypes.GET_INCOMING_REPORT],
  );
  ...
  const refresh = () =>
    dispatch(
      incomingReportActions.getIncomingReportFetch(
        listOptions as IncomingReportListPayload,
      ),
    );
      ...
      <Table
        ...
        customSearch={
          isMobile ? (
            <MobileTableHeader
              title={t("table.title")}
              selectId="incoming-report-search-by-mobile"
...
export default IncomingReportInitialPage;
