...
/** Binning slip print — headless (tanpa popup): saat open, fetch data lalu buka
 *  dokumen print tab baru. Layout parity legacy BinningSlip
 *  (PlanIncomingController.cs): logo + judul BINNING SLIP biru #003399,
 *  header PT SerasiLogistics, section CUSTOMER, tabel 8 kolom, catatan kaki
 *  (TANPA kotak tanda tangan — beda dengan picking slip). */
const BinningSlipForm = ({ open, headerId, onClose }: Props) => {
  useEffect(() => {
    ...
    const print = async () => {
      ...
      try {
        rows =
          (await OutstandingIncomingApi().retrieveBinningSlip(headerId)) ?? [];
      } catch {
        onClose();
        return;
      }
      if (!rows.length) {
        // parity legacy: partial qty 0 → tidak ada dokumen
        message.info(
          "No data available to generate PDF because the partial qty is 0.",
        );
        onClose();
        return;
      }
      ...
      win.document.write(`<!DOCTYPE html><html><head><title>BINNING SLIP</title>
<style>
